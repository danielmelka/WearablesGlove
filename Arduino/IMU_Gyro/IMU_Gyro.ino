...
void loop() {
  ...
  accAngleX = (atan(AccY / sqrt(pow(AccX, 2) + pow(AccZ, 2))) * 180 / PI) +0.07; // AccErrorX ~(-0.07) See the calculate_IMU_error()custom function for more details
  accAngleY = (atan(-1 * AccX / sqrt(pow(AccY, 2) + pow(AccZ, 2))) * 180 / PI) +0.01; // AccErrorY ~(-0.01)
  // === Read gyroscope data === //
  ...
  GyroX = GyroX - 4.45; // GyroErrorX ~(4.45)
  GyroY = GyroY - 2.45; // GyroErrorY ~(2.45)
  GyroZ = GyroZ - 0.53; // GyroErrorZ ~ (0.53)
